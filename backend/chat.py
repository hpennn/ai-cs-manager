...
class CreateSessionRequest(BaseModel):
    channel: str = "web"  # "wechat" | "douyin" | "web" | "custom"
    merchant_name: str = "未命名"
    description: str = ""
    login_url: str = ""
    mode: str = "auto"
...
async def create_session(request: CreateSessionRequest):
    ...
    try:
        index = load_sessions_index()
        index.append(session_record)
        save_sessions_index(index)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"保存会话索引失败: {str(e)}")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"创建对话记录文件失败: {str(e)}")
...
async def list_sessions():
    """获取所有会话列表（含 mode、last_message、customer_name、channel、merchant_name 等）"""
    index = load_sessions_index()
    ...
    for item in index:
        ...
        if "channel" not in item:
            item["channel"] = "web"
        if "merchant_name" not in item:
            item["merchant_name"] = "未命名"
        if "description" not in item:
            item["description"] = ""
        if "login_url" not in item:
            item["login_url"] = ""
...
async def get_session_detail(session_id: str):
    """获取会话详情（含 mode、channel、merchant_name 等）"""
    index = load_sessions_index()
    session_info = None
    for item in index:
        if item["session_id"] == session_id:
    ...
    # 向后兼容：确保新增字段存在
    if "channel" not in session_info:
        session_info["channel"] = "web"
    if "merchant_name" not in session_info:
        session_info["merchant_name"] = "未命名"
    if "description" not in session_info:
        session_info["description"] = ""
    if "login_url" not in session_info:
